...
df = pd.read_csv(link).drop(columns=['Nota','Avaliacao','Genero'],axis=1)#Excluindo Colunas
...
if filtro1 and filtro2 and filme =="":
    csv = convert_df(df1)
    st.write('')
    st.write(f"### *Dá um olhada nos {len(df1)} filmes que filtrei para você* :sunglasses:")
